...
const SUPABASE_ANON_KEY =
  localStorage.getItem("sb_key") || DEFAULT_SUPABASE_ANON_KEY;
...
function triggerScannerCooldown(durationMs = SCAN_COOLDOWN) {
  ...
  text.innerHTML =
    '<span class="live-dot" style="background: var(--gold); width: 7px; height: 7px;"></span> Jeda Scan (Cooldown)';
  ...
  cooldownTimerInterval = setInterval(() => {
    ...
    if (remainingMs <= 0) {
      ...
      text.innerHTML =
        '<span class="live-dot" style="background: var(--green); width: 7px; height: 7px;"></span> Scanner Siap';
